...
def _is_field_subsuite(suite_name: str) -> bool:
    ...
    Matches both:
      - 'Kiểm tra trường X'  (full form)
      - 'Trường X'           (short form generated by skill)
    """
    import re as _re
    s = suite_name.strip()
    return bool(
        _re.match(r'kiểm tra trường\s+\S', s, _re.IGNORECASE) or
        _re.match(r'^trường\s+\S', s, _re.IGNORECASE)
    )
