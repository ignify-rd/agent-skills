...
description: >
  ...
  fill the Actual Result column, evaluate Pass/Fail using Claude AI, replace the sample JSON
  in Expected Result with the real response JSON, embed screenshots into an "Evidence" sheet,
  and upload the merged xlsx back to Google Sheets.
---
...
3. Fills the **Actual Result** column with the verdict: `PASS` / `FAIL` / `BLOCKED`
4. Evaluates **Pass / Fail / Blocked** using Claude AI (comparing expected vs actual)
5. Writes **only** the verdict (`PASS` / `FAIL` / `BLOCKED`) to the **Status** column
...
| `status` | **optional** | Filled with `PASS` / `FAIL` / `BLOCKED`. Auto-detected if missing. |
...
4. **Report results**: matched count, PASS/FAIL/BLOCKED breakdown, unmatched test cases, Google Sheet URL
