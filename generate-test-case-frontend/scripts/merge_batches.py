...
def collect_batch_files(output_dir: str) -> list:
    ...
    # 2. batch-search.json (optional — LIST screens only)
    p = os.path.join(output_dir, "batch-search.json")
    if os.path.exists(p):
        files.append(p)
    ...
    # 5. batch-workflow.json (optional — maker-checker + role flows)
    p = os.path.join(output_dir, "batch-workflow.json")
    if os.path.exists(p):
        files.append(p)
