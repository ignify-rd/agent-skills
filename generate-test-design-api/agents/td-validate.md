...
<step id="1b" name="Detect field source and load appropriate templates">
    <condition>Check FIELD_SOURCE parameter</condition>
    <actions>
        <action type="branch">
            <case value="fileContentFields">
                <description>Load FileContentField templates from api-test-design.md</description>
                <script>python3 {SKILL_SCRIPTS}/search.py --ref api-test-design --section "FileContentField TextInput,FileContentField NumberInput,FileContentField DateInput,FileContentField Droplist,FileContentField CrossField,FileContentField output-format"</script>
            </case>
            <case value="fieldConstraints" default="true">
                <description>Use standard field type templates (existing behavior)</description>
            </case>
        </action>
    </actions>
</step>

<step id="2" name="Get field data from inventory via scripts (NOT read full file)">
    <description>
        ⛔ KHÔNG đọc toàn bộ inventory.json — dùng inventory.py get để chỉ lấy đúng data cần thiết.
        Điều này tiết kiệm tokens đáng kể khi inventory có nhiều fields.
    </description>

    <actions>
        <action type="bash" name="Get field constraints for this batch">
            <script_fieldConstraints>python3 {SKILL_SCRIPTS}/inventory.py get \
  --file {INVENTORY_FILE} \
  --category fieldConstraints \
  --fields {FIELD_NAMES_CSV}</script_fieldConstraints>
            <script_fileContentFields>python3 {SKILL_SCRIPTS}/inventory.py get \
  --file {INVENTORY_FILE} \
  --category fileContentFields \
  --fields {FIELD_NAMES_CSV}</script_fileContentFields>
            <note>Use script_fieldConstraints when FIELD_SOURCE = "fieldConstraints", script_fileContentFields when FIELD_SOURCE = "fileContentFields". FIELD_NAMES_CSV = comma-separated field names from FIELD_BATCH.</note>
        </action>

        <action type="bash" name="Get validate error codes">
            <script>python3 {SKILL_SCRIPTS}/inventory.py get \
  --file {INVENTORY_FILE} \
  --category errorCodes \
  --filter section=validate \
  --keys code,desc,trigger</script>
        </action>

        <action type="bash" name="Get response schemas">
            <script>python3 {SKILL_SCRIPTS}/inventory.py get \
  --file {INVENTORY_FILE} \
  --category responseSchema \
  --keys success,error</script>
    ...
    <extract_from_results>
        <item>fieldConstraints/fileContentFields → rsdConstraints per field (or displayName, inputType, allowedChars, crossFieldRules, businessValidation for file content)</item>
...
<step id="3" name="Generate validate per field (in order)">
    ...
    <general_rules>
        <rule>
            ...
            <file_content_heading_format>### Trường {displayName}</file_content_heading_format>
            <note>NO type or Required/Optional suffix in heading. For fileContentFields, use displayName (Vietnamese)</note>
        </rule>
        <rule name="file_content_case_heading">
            <condition>When FIELD_SOURCE = "fileContentFields"</condition>
            <format>- Kiểm tra truyền file hợp lệ, nội dung file {mô tả case}</format>
            <example_correct>- Kiểm tra truyền file hợp lệ, nội dung file Để trống Tài khoản chuyển</example_correct>
            <example_correct>- Kiểm tra truyền file hợp lệ, nội dung file Nhập Tài khoản chuyển 15 ký tự</example_correct>
            <example_correct>- Kiểm tra truyền file hợp lệ, nội dung file Nhập mã số thuế người nộp thuế là các ký tự đặc biệt ngoài các ký tự @&-()+_,/</example_correct>
        </rule>
        <rule name="file_content_response">
            <condition>When FIELD_SOURCE = "fileContentFields"</condition>
            <description>
                File content fields are validated by backend job (async), NOT by the upload API directly.
                Response format describes record status, NOT API error code:
                - Error: "Bản ghi không hợp lệ, mô tả lỗi: {errorMessage from businessValidation/crossFieldRule}"
                - Success: "Bản ghi hợp lệ"
                Status luôn = 200 (vì API upload trả success, job xử lý async)
            </description>
        </rule>
        <rule>
...
<step id="4" name="Per-field checkpoint (IMMEDIATELY after each field)">
    ...
    <checkpoint_categories per_type="FileContentField TextInput Required">
        Để trống ✓ | All space ✓ | maxLength-1 ✓ | maxLength ✓ | maxLength+1 ✓ | Ký tự đặc biệt cho phép ✓ | Ký tự đặc biệt không cho phép ✓ | Dấu tiếng Việt ✓ | Space/tab/khoảng trắng kép ✓ | Leading zero ✓ | Space ở giữa ✓ | crossFieldRules ✓ | businessValidation ✓
    </checkpoint_categories>

    <checkpoint_categories per_type="FileContentField NumberInput Required">
        Để trống ✓ | All space ✓ | Boundary max ✓ | Boundary max+1 ✓ | Boundary max-1 ✓ | Chữ ✓ | Ký tự đặc biệt ✓ | Số âm ✓ | Số thập phân ✓ | Leading zero ✓ | businessValidation (hạn mức) ✓
    </checkpoint_categories>

    <checkpoint_categories per_type="FileContentField DateInput Required">
        Để trống ✓ | Sai format ✓ | Ngày hiện tại ✓ | Ngày tương lai ✓ | Ngày quá khứ ✓
    </checkpoint_categories>

    <checkpoint_categories per_type="FileContentField Droplist Required">
        Để trống ✓ | All space ✓ | Không có trong danh sách ✓ | Có trong danh sách nhưng không thuộc parent ✓ | Hợp lệ ✓ | businessValidation ✓
    </checkpoint_categories>
...
- Kiểm tra truyền file hợp lệ, nội dung file Nhập Tài khoản chuyển 15 ký tự

    - 1. Check api trả về:
      1.1.Status: 200
      1.2.Response: Bản ghi không hợp lệ, mô tả lỗi: "Trường Tài khoản chuyển vượt quá độ dài cho phép"

### Trường Mã Số thuế người nộp thuế

- Kiểm tra truyền file hợp lệ, nội dung file Để trống mã số thuế người nộp thuế

    - 1. Check api trả về:
      1.1.Status: 200
      1.2.Response: Bản ghi không hợp lệ, mô tả lỗi: "Trường Mã số thuế người nộp thuế không được để trống"

- Kiểm tra truyền file hợp lệ, nội dung file Nhập mã số thuế người nộp thuế là các ký tự đặc biệt ngoài các ký tự @&-()+_,/

    - 1. Check api trả về:
      1.1.Status: 200
      1.2.Response: Bản ghi không hợp lệ, mô tả lỗi: "Trường ... không cho phép ký tự đặc biệt trừ @&()-._ ,/"
```
    </example_file_content_valid>
</step>
...
<task_context>
    <parameters>
        ...
        <param name="FIELD_TYPES_NEEDED" type="string" required="true">
            ...
            <example_file_content>FileContentField TextInput,FileContentField Droplist</example_file_content>
        </param>
        <param name="FIELD_SOURCE" type="string" default="fieldConstraints">
            <description>Source of field definitions: "fieldConstraints" (API params) or "fileContentFields" (fields inside uploaded file)</description>
        </param>
